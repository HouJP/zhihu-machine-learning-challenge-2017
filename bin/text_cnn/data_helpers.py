...
def load_dataset(tc_vecs, tw_vecs, cc_vecs, cw_vecs, btm_vecs, word_share_vecs, lid_vecs, inds):
    ...
    sub_word_share_vecs = None if word_share_vecs is None else np.asarray([word_share_vecs[ind] for ind in inds],
                                                                          dtype='float32')
    sub_lid_vecs = None if lid_vecs is None else np.asarray([lid_vecs[ind] for ind in inds], dtype='int32')
    return sub_tc_vecs, sub_tw_vecs, sub_cc_vecs, sub_cw_vecs, sub_btm_vecs, sub_word_share_vecs, sub_lid_vecs


def load_dataset_loop(tc_vecs, tw_vecs, cc_vecs, cw_vecs, btm_vecs, word_share_vecs, lid_vecs, inds, part_size):
    count = 0
    inds_len = len(inds)
    inds_part = list()
    while True:
        ...
        if 0 == count % part_size:
            yield load_dataset(tc_vecs, tw_vecs, cc_vecs, cw_vecs, btm_vecs, word_share_vecs, lid_vecs, inds_part)
            inds_part = list()
