...
def load_dataset_from_file(config, data_name, word_emb_index, char_emb_index, inds):
    ...
    fs_idf_100_pl_fp = '%s/%s.%s.csv' % (config.get('DIRECTORY', 'dataset_pt'), 'fs_idf_100_pl', data_name)
    ...
    sub_fs_idf_100_pl = np.asarray(load_feature_vec_part(fs_idf_100_pl_fp, inds_copy, inds_map), dtype='float32')
    ...
    return [sub_tw_vecs, sub_cw_vecs, sub_tc_vecs, sub_cc_vecs, sub_fs_btm_tw_cw, sub_fs_btm_tc, sub_fs_idf_100_pl, sub_lid_vecs]
